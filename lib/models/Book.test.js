...
describe('Book model', () => {
    ...
    describe('totalPages', () => {
        it('requires a totalPages field', () => {
            const book = new Book({
                author: 'H.P. Lovecraft',
                title: 'Great Tales of Horror',
                category: 'Horror Fiction',
            ...
            const { errors } = book.validateSync();
            expect(errors.totalPages.message).toEqual('Path `totalPages` is required.');
        });
        ...
        it('is over 0', () => {
            ...
            const { errors } = book.validateSync();
            expect(errors.price.message).toEqual('Path `price` (0) is less than minimum allowed value (1).');
        });
    });
